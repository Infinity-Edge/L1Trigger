...
L1TPFProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
    ...
    for (const auto & tag : hadCands_) {
        ...
        for (unsigned int ic = 0, nc = calos.size(); ic < nc; ++ic) {
            ...
            if( ic % 100 ==  0 )
            {
                cout << ic+1 << "th calo"<< endl;
                cout << "calo.pt : " << calo.pt() <<", hadPtCut_ : "<< hadPtCut_ << ", calo.eta : " << calo.eta()<<endl;
            }
            if (fabs(calo.eta()) < 5 && fabs(calo.eta()) > 3)
            {
                NCaloBe ++;
                if (calo.pt() > hadPtCut_) NCaloAf ++;
            }
        }
    }
    cout << "Number of calo clusters before HF threshold : " << NCaloBe << endl;
    cout << "Number of calo clusters after HF threshold : " << NCaloAf << endl;
